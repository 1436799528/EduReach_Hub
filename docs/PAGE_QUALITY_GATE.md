...
* EduReach colors (Primary Academic Crimson `#B8492F`, Deep Crimson `#97300F`, Institutional Navy `#0F172A`, Secondary Tangerine `#C9641E`, Borders `#e2e8f0`) — see `docs/COLOR_THEME_SPEC.md` as the single source of truth. Success Green `#16A34A` and Trust Gold `#CA8A04` are reserved for status/verification only, never as primary brand color.
