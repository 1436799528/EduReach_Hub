...
| `/cbt/setup/jamb`, `/waec`, `/neco`, `/post-utme` | **PASS** | Maintained course/subject/school catalogues, duplicate prevention, official-brochure reminders and session-stored form progress are present. Setup links go directly to the timed hall. |
| `/cbt/practice` | **PASS WITH DATA DEPENDENCY** | Timed exam bar, server-backed attempt ownership/scoring, resumable signed-in answers and current question position, guest mode, question palette, keyboard controls, flags, auto-submit, IndexedDB progress and scientific on-screen calculator are present. Production question delivery requires configured data; answer keys are not delivered before submission. |
...
| `/jobs`, `/scholarships` | **PASS WITH DATA DEPENDENCY** | Opportunities and grants are a first-class route/nav/footer destination. The current configured list is small and the Scholarships & Grants filter has an honest verified-unavailable/notify state rather than invented listings; category state survives URL/back navigation. |
...
| `/schools`, `/schools/:slug` | **PASS WITH DATA DEPENDENCY** | Maintained institution search covers names, acronyms, locations, types and configured course tags, with autocomplete, shortlist support and useful detail pages. Remote data remains limited to configured institution rows. |
...
| `/admission`, `/admission/*` | **CONTROLLED COMING SOON** | No invented admission coverage; calculator, School Finder and verified updates remain available. |
...
| Services → guided form → review → submit → reference → My Requests | **PASS WITH DATA DEPENDENCY** | Four supported service keys, populated institution catalogue, form validation, production Auth/RLS insert and dashboard-only history with status, description, date, reference and WhatsApp continuation are wired. Local mode is browser-scoped only. |
| Search/filter/form state → back navigation → same context | **PASS** | Services, library, news, opportunities, School Finder and CBT setup preserve URL/session state; CBT preserves unfinished answers and question position locally and, for signed-in attempts, through the server-managed attempt. |
...
14. **CBT resume position:** `cbt_attempts.current_question` and migration `20260925213000_cbt_attempt_question_position.sql` preserve the zero-based question position for active signed-in attempts; completed attempts remain excluded from resume lookup. The browser IndexedDB state carries the same position for local/guest continuation.
15. **Request handling states:** the authenticated dashboard and admin queue recognize awaiting-information and closed statuses while keeping WhatsApp as a continuation channel rather than a replacement for the EduReach request record.
...
Completed on the current workspace after the release fixes and the CBT/profile/school/request follow-up pass:
...
- Static source scan — no `href="#"`, TODO/FIXME or lorem-ipsum placeholders in application source; the scan is heuristic and does not replace browser testing.
- Development server smoke checks — `/`, `/schools`, `/schools/university-of-lagos`, `/profile?edit=1`, `/services`, `/past-questions?view=materials&exam=JAMB`, `/jobs?category=scholarship`, `/cbt/setup/jamb`, `/cbt/practice?exam=practice-exam-jamb`, `/cbt/results?attempt=guest-cbt-test`, `/dashboard/services?ref=ER-2026-TEST` and `/admin/queue` returned SPA **200** responses.
- Development API smoke checks — `/api/health` **200**, unauthenticated `/api/admin/session` **401**, unauthenticated `/api/cbt/exams/foo/questions` **404**, and unknown `/api/not-real` **404 JSON**.
- Production-mode smoke server from the generated `dist/server.cjs` — the same deep links returned **200**, health returned **200**, unknown API returned **404 JSON**, and security headers were present.
- Calculator-engine assertions — the safe calculator engine assertions (`2+3*4`, `2^3^2`, `sin(30)`, `2(3+1)`, `-2^2`, `sqrt(9)`, `15%`) passed; static `/icons/*` and `/news/photos/*` references had **0 missing assets**.
...
1. Apply all migrations, including `20260925213000_cbt_attempt_question_position.sql` and `20260925214000_service_request_statuses.sql`, through `20260925_public_release_security_hardening.sql` to the production Supabase project; verify the own-request/profile permissions with a real student account and verify an admin cannot be demoted through the client.
