  ...
  return (
    <Shell title="News &amp; Updates">
      <section className="blog_area section-padding"><div className="container">
        ...
        <div className="row">{filteredPosts.map((post) => <div className="col-lg-4 col-sm-6 col-xs-12" key={post.id}><article className="blog_post"><div className="blog-img"><img src={post.image} alt={post.title} loading="lazy" /></div><div className="blog_content"><div className="blog_meta"><span>{post.date}</span><span>{post.category}</span>{post.badge && <span>{post.badge}</span>}</div><h3>{post.title}</h3><p>{post.excerpt}</p><div className="blog-card-footer"><span>{post.author}</span><span>{post.readTime}</span></div></div></article></div>)}</div>
