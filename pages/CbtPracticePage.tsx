...
const EXAM_ID = 'JAMB_USE_OF_ENGLISH_2026';
...
export default function CbtPracticePage() {
  ...
  useEffect(() => {
    getExamProgress(EXAM_ID).then((saved) => {
      if (saved) { setAnswers(saved.answers); setFlags(saved.flags); setSeconds(saved.timeRemainingSeconds); }
      setRestored(true);
  ...
  useEffect(() => {
    ...
    const onOnline = () => { setOffline(false); void syncPendingSubmissions(async (payload) => { const response = await fetch('/api/cbt/submit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); return response.ok; }); };
    window.addEventListener('keydown', onKeyDown); window.addEventListener('offline', onOffline); window.addEventListener('online', onOnline);
    return () => { window.removeEventListener('keydown', onKeyDown); window.removeEventListener('offline', onOffline); window.removeEventListener('online', onOnline); };
  ...
  }, [answers, flags, seconds, restored]);

  async function submitExam() {
    setMessage('');
    if (offline) {
      await queueOfflineSubmission({ examId: EXAM_ID, answers, timeSpentSeconds: 20 * 60 - seconds });
      setMessage('Exam submission is queued. It will sync automatically when your connection returns.');
      return;
    }
    const response = await fetch('/api/cbt/submit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ examId: EXAM_ID, answers, timeSpentSeconds: 20 * 60 - seconds }) });
    if (!response.ok) setMessage('Submission could not be completed. Your progress is still saved locally.');
    else window.location.href = '/cbt/results';
  }
  ...
  return <HubLayout><div className="hub-cbt-engine">
    <div className="hub-cbt-topbar"><div><strong>EduReach CBT</strong><span>Use of English · JAMB 2026</span></div><div className="hub-cbt-timer"><TimerReset size={17}/> {mins}:{secs}</div><div className="hub-cbt-network">{offline && <><WifiOff size={15}/> Offline mode • progress saved locally</>}</div><button className="hub-primary-btn" onClick={() => void submitExam()}><Send size={16}/> {offline ? 'Queue Submission' : 'Submit Exam'}</button></div>
