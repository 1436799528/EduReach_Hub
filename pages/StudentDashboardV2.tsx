  ...
  return <div className="student-portal">
    ...
    <main className="student-portal-main">{loading ? <div className="student-loading">Loading student workspace…</div> : <div className="student-grid">
      <aside className="student-sidebar">
        ...
        <section className="student-card"><div className="card-title-row"><h3>Service Requests</h3><span>{requests.length}</span></div><div className="service-mini-list service-mini-list-icons">{requests.slice(0, 3).map(req => <div key={req.id}><span className="service-list-mark"><CardIdentityMark value={serviceMap[req.service_id]?.service_key || 'services'} type="service" /></span><strong>{String(req.form_data?.serviceTitle || serviceMap[req.service_id]?.title || 'Service request')}</strong><small>{statusLabel(req.status)}</small></div>)}{!requests.length && <p className="empty-copy">No service requests yet.</p>}</div></section>
      </aside>

      <section className="student-workspace">
        <section className="student-card action-card"><div className="card-title-row"><div><h3>Quick Actions</h3><p>Common student tasks.</p></div></div><div className="quick-actions-grid">
<a href="/cbt" className="quick-action quick-blue"><CardIdentityMark value="jamb-slip" type="service" /><b>Practice Center</b><small>CBT</small></a>
<a href="/services" className="quick-action quick-green"><CardIdentityMark value="services" type="service" /><b>Student Services</b><small>Services</small></a>
<a href="/services/track" className="quick-action quick-amber"><CardIdentityMark value="upcoming-deadline" type="upcoming" /><b>Request Tracker</b><small>Status</small></a>
<a href="/news" className="quick-action quick-slate"><CardIdentityMark value="news" type="content" /><b>Latest Updates</b><small>News</small></a>
</div></section>
