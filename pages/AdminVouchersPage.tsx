...
export default function AdminVouchersPage() {
  ...
  return <AdminLayout><div className="admin-page"><div className="admin-page-header"><div><h1>Scratch Card Voucher Inventory</h1><p>PINs are never loaded with the normal inventory list. Reveal requires an explicit protected action.</p></div></div>{error && <div className="admin-card" role="alert" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}><span>{error}</span><button type="button" className="admin-btn small" onClick={() => void load()}>Try again</button></div>}<div className="admin-kpi-grid two"><div className="admin-kpi"><span>{body} {year} Available</span><strong>{available}</strong><small>Ready for dispensing</small></div><div className="admin-kpi"><span>Total Loaded</span><strong>{vouchers.length}</strong><small>Current filter</small></div></div><div className="admin-card"><div className="admin-card-header"><h2>Add Voucher</h2><span>One at a time for controlled stock entry</span></div><div className="admin-inline-form"><select aria-label="Voucher examination body" className="admin-select" value={body} onChange={e=>setBody(e.target.value)}><option>WAEC</option><option>NECO</option></select><select aria-label="Voucher examination year" className="admin-select" value={year} onChange={e=>setYear(e.target.value)}><option>2026</option><option>2025</option><option>2024</option></select><input className="admin-input" aria-label="Voucher serial number" placeholder="Serial number" value={serial} onChange={e=>setSerial(e.target.value)}/><input className="admin-input" aria-label="Voucher PIN" placeholder="PIN" value={pin} onChange={e=>setPin(e.target.value)}/><button type="button" className="admin-btn success" onClick={()=>void addVoucher()}>Add to Inventory</button></div></div><div className="admin-card"><div className="admin-card-header"><h2>Inventory Log</h2><span>PIN reveal is individually audited</span></div><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Card Type</th><th>Serial</th><th>PIN</th><th>Status</th><th>Loaded</th></tr></thead><tbody>{vouchers.map(v=><tr key={v.id}><td>{v.exam_body} {v.exam_year}</td><td className="mono">{v.serial_number}</td><td className="mono sensitive">{revealed[v.id] || '••••••••••••'} {!revealed[v.id] && <button type="button" className="admin-text-btn" onClick={()=>void reveal(v.id)}>Reveal</button>}</td><td><span className={`status-badge ${v.status}`}>{v.status}</span></td><td>{new Date(v.created_at).toLocaleString()}</td></tr>)}{!vouchers.length && <tr><td colSpan={5} className="empty-state">No vouchers in this inventory.</td></tr>}</tbody></table></div></div></div></AdminLayout>;
}
