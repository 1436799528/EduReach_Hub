...
export default function ServiceApplyPage({ slug }: { slug: string }) {
  ...
  function submit(event: FormEvent) { event.preventDefault(); if (step < 3) setStep(step + 1); else void handleSubmit(); }
  ...
  return <HubLayout><div className="hub-page"><div className="hub-container hub-narrow">
    <div className="hub-page-title"><span className="hub-eyebrow">SERVICE WIZARD</span><h1>{service.title}</h1><p>Complete the three steps. Payment integration is ready to connect to the approved payment provider.</p></div>
    ...
    {reference ? <div className="hub-success-panel"><span className="hub-success-icon"><Check size={24}/></span><div><span className="hub-eyebrow">REQUEST RECEIVED</span><h2>{message}</h2><p>Your reference code is <strong>{reference}</strong>. Keep it safe for tracking.</p><a className="hub-primary-btn" href={`/services/track?ref=${encodeURIComponent(reference)}`}>Track Request</a></div></div> : <form className="hub-panel hub-wizard-panel" onSubmit={submit}>
      ...
      {step === 3 && <div><h2>Payment & submission</h2><div className="hub-payment-summary"><div><span>Service</span><strong>{service.title}</strong></div><div><span>Fee</span><strong>{service.price}</strong></div></div><div className="hub-payment-box"><CreditCard size={22}/><div><h3>Paystack integration point</h3><p>The request is recorded only after the secure service workflow confirms submission. Payment can be attached to this step later without exposing card details to EduReach.</p></div></div>{message && <div className="hub-form-error">{message}</div>}</div>}
      <div className="hub-wizard-actions">{step > 1 && <button type="button" className="hub-outline-btn" onClick={()=>setStep(step - 1)}><ChevronLeft size={16}/> Back</button>}<span />{step < 3 ? <button type="submit" className="hub-primary-btn" disabled={!canNext}>Continue <ChevronRight size={16}/></button> : <button type="submit" className="hub-primary-btn" disabled={busy}>{busy ? 'Submitting…' : 'Submit Request'}</button>}</div>
    </form>}
  </div></div></HubLayout>;
}
