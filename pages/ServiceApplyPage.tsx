...
export default function ServiceApplyPage({ slug }: { slug: string }) {
  ...
  return <HubLayout><div className="hub-page"><div className="hub-container hub-narrow">
    ...
    </form> : <div className="hub-success-panel"><span className="hub-success-icon"><Check size={24}/></span><div><span className="hub-eyebrow">REQUEST RECEIVED</span><h2>{message}</h2><p>Reference: <strong>{reference}</strong></p><div className="hub-wizard-actions"><a className="hub-outline-btn" href="/services">Services</a><a className="hub-primary-btn" href={`/services/track?ref=${encodeURIComponent(reference)}`}>Track Request <ChevronRight size={16}/></a></div></div></div>}</>}
  </div></div></HubLayout>;
}
