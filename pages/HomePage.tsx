...
export default function HomePage() {
  ...
  return (
    <Shell>
      ...
      <section id="services" className="cat_area section-padding"><div className="container"><div className="section-title text-center"><h2>What EduReach Provides</h2><p>Choose the practical student service you need.</p></div><div className="row">{services.map((service) => <div className="col-lg-4 col-md-6 col-sm-6 col-xs-12" key={service.id}><div className="single_cat"><img src={service.image} alt={service.title} loading="lazy" /><div className="single_cat_text"><h4>{service.title}</h4><p>{service.description}</p><a href={`/services/${service.id}`}>View Service <i className="fa fa-angle-right" /></a></div></div></div>)}</div></div></section>

      <section className="course_area section-padding"><div className="container"><div className="section-title text-center"><h2>Explore EduReach Services</h2><p>Practical services designed around everyday student needs.</p></div><div className="row">{services.map((service) => <div className="col-lg-4 col-sm-6 col-xs-12" key={service.id}><div className="single_course"><div className="single_c_img"><img src={service.image} className="img-fluid" alt={service.title} loading="lazy" /><span>{service.shortTitle}</span></div><div className="single_course_text"><span className={service.icon} style={{ fontSize: 28 }} /><h4>{service.title}</h4><p>{service.description}</p><a href={`/services/${service.id}`} className="btn_one">{service.cta}</a></div></div></div>)}</div></div></section>
      ...
      <section id="blog" className="blog_area section-padding"><div className="container"><div className="section-title text-center"><h2>Latest News &amp; Campus Updates</h2><p>News, campus gist, quick updates, academic guidance, admissions and student opportunities.</p></div><div className="row">{blogPosts.slice(0, 6).map((post) => <div className="col-lg-4 col-sm-6 col-xs-12" key={post.id}><article className="blog_post"><a href={`/blog/${post.id}`} className="blog-img"><img src={post.image} alt={post.title} loading="lazy" /></a><div className="blog_content"><div className="blog_meta"><span>{post.date}</span><span>{post.category}</span>{post.badge && <span>{post.badge}</span>}</div><h3><a href={`/blog/${post.id}`}>{post.title}</a></h3><p>{post.excerpt}</p><a href={`/blog/${post.id}`} className="blog_readmore">Read More <i className="fa fa-long-arrow-right" /></a></div></article></div>)}</div></div></section>
