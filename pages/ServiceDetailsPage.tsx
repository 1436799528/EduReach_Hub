...
export default function ServiceDetailsPage({ id }: { id: string }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (authError || !authData.user) {
      setMessage('Please sign in before submitting a service request.');
      setSubmitting(false);
      setAuthMode('signin');
      return;
    }
    ...
    if (serviceError || !serviceRow) {
      setMessage('This service is not available for submission right now. Please try again later.');
      setSubmitting(false);
      return;
    }
    ...
    if (error) {
      setMessage(error.message || 'We could not submit your request. Please try again.');
      setSubmitting(false);
      return;
    }
    ...
    setSubmitted(true);
    setMessage('Your request has been submitted successfully. You can continue using EduReach while it is reviewed.');
    setSubmitting(false);
  }
            ...
            <div className="col-lg-5">
              <div className="contact_info service-request-card">
                <div className="section-title">
                  ...
                  <p>Submit your details and EduReach will create a service request for follow-up.</p>
                </div>
                <form className="contact-form" onSubmit={handleSubmit}>
                  {detail.fields.map((field) => field.name === 'message' || field.name === 'reason' ? (
                    <div className="col-md-12" key={field.name}>
                      <label htmlFor={field.name}>{field.label}</label>
                      <textarea id={field.name} name={field.name} className="form-control" rows={5} placeholder={field.placeholder} required={field.required} />
                    </div>
                  ) : (
                    <div className="col-md-12" key={field.name}>
                      <label htmlFor={field.name}>{field.label}</label>
                      <input id={field.name} name={field.name} type={field.type || 'text'} className="form-control" placeholder={field.placeholder} required={field.required} />
                    </div>
                  ))}
                  <div className="col-md-12">
                    <button className="btn_one" type="submit" disabled={submitting}>
                      {submitting ? 'Submitting…' : 'Submit Request'}
                    </button>
                  </div>
                </form>
                {message && <div className={`form-success ${submitted ? 'service-request-success' : 'service-request-error'}`}>{message}</div>}
                {!submitted && <button type="button" className="service-signin-link" onClick={() => setAuthMode('signin')}>Already have an EduReach account? Sign in</button>}
