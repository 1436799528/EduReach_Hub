            ...
            <div className="er-events-list">
              {events.map((item) => (
                <article className={`er-event-card ${identityClassFor(`${item.kind} ${item.title}`, 'upcoming')}`} key={item.id}>
                  ...
                  <div className="er-event-copy"><span className="er-library-badge">{item.kind === 'exam' ? 'Exam event' : 'Deadline'}</span><h2>{item.title}</h2>{looksLikeHtml(item.description || '') ? <div className="er-event-rich" dangerouslySetInnerHTML={{ __html: sanitizeRichHtml(item.description || '') }} /> : <p>{item.description || 'Check the official notice for the full event details.'}</p>}<div className="er-event-meta"><span><Clock3 size={13} /> {eventDate(item)}</span>{item.location && <span><MapPin size={13} /> {item.location}</span>}</div></div>
                </article>
              ))}
            </div>
          )}
