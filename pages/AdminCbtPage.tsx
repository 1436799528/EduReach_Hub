...
export default function AdminCbtPage() {
  ...
  return <AdminLayout><div className="admin-page">
    <div className="admin-page-header"><div><h1>CBT Exam Manager</h1><p>Manage the production CBT engine: exams, question order, answer keys and explanations.</p></div><button className="admin-btn success" onClick={() => setShowExamForm(true)}>+ New Exam</button></div>
    {message && <div className="admin-card">{message}</div>}
    {showExamForm && <form className="admin-card admin-question-form" onSubmit={saveExam}><div className="admin-card-header"><h2>Create Exam</h2><button type="button" className="admin-text-btn" onClick={() => setShowExamForm(false)}>Close</button></div><div className="admin-inline-form"><input className="admin-input" placeholder="Exam title" value={examForm.title} onChange={e=>setExamForm({...examForm,title:e.target.value})} required/><select className="admin-select" value={examForm.exam_body} onChange={e=>setExamForm({...examForm,exam_body:e.target.value})}><option>JAMB</option><option>WAEC</option><option>NECO</option><option>POST-UTME</option><option>EduReach</option></select><input className="admin-input" placeholder="Subject" value={examForm.subject} onChange={e=>setExamForm({...examForm,subject:e.target.value})} required/><input className="admin-input" type="number" min="5" max="180" value={examForm.duration_minutes} onChange={e=>setExamForm({...examForm,duration_minutes:e.target.value})}/></div><textarea className="admin-textarea" placeholder="Description" value={examForm.description} onChange={e=>setExamForm({...examForm,description:e.target.value})}/><button className="admin-btn" disabled={saving}>{saving ? 'Saving…' : 'Create Exam'}</button></form>}
    <div className="admin-filter-card"><select className="admin-select" value={selectedExam} onChange={e=>setSelectedExam(e.target.value)}><option value="">Select an exam</option>{exams.map(e=><option key={e.id} value={e.id}>{e.exam_body} · {e.subject} · {e.title}</option>)}</select>{exam && <span className="admin-filter-count">{exam.duration_minutes} minutes · {questions.length} questions</span>}</div>
    {selectedExam && <div className="admin-page-header"><div><h2>{exam?.title}</h2><p>{exam?.description || 'No description.'}</p></div><button className="admin-btn" onClick={()=>startQuestion()}>+ Add Question</button></div>}
    {showQuestionForm && <form className="admin-card admin-question-form" onSubmit={saveQuestion}><div className="admin-card-header"><h2>{editingQuestion ? 'Edit Question' : 'Add Question'}</h2><button type="button" className="admin-text-btn" onClick={()=>setShowQuestionForm(false)}>Close</button></div><textarea className="admin-textarea" placeholder="Question text" value={questionForm.question_text} onChange={e=>setQuestionForm({...questionForm,question_text:e.target.value})} required/><div className="admin-option-grid">{(['a','b','c','d'] as const).map(letter=><input key={letter} className="admin-input" placeholder={`Option ${letter.toUpperCase()}`} value={questionForm[`option_${letter}`]} onChange={e=>setQuestionForm({...questionForm,[`option_${letter}`]:e.target.value})} required/>)}</div><div className="admin-inline-form"><select className="admin-select" value={questionForm.correct_option} onChange={e=>setQuestionForm({...questionForm,correct_option:e.target.value as 'A'|'B'|'C'|'D'})}><option>A</option><option>B</option><option>C</option><option>D</option></select><input className="admin-input" type="number" min="1" value={questionForm.marks} onChange={e=>setQuestionForm({...questionForm,marks:e.target.value})}/><input className="admin-input" type="number" min="1" value={questionForm.position} onChange={e=>setQuestionForm({...questionForm,position:e.target.value})}/></div><input className="admin-input" placeholder="Explanation (optional)" value={questionForm.explanation} onChange={e=>setQuestionForm({...questionForm,explanation:e.target.value})}/><button className="admin-btn" disabled={saving}>{saving ? 'Saving…' : 'Save Question'}</button></form>}
    {loading ? <div className="admin-card empty-state">Loading CBT exams…</div> : !exams.length ? <div className="admin-card empty-state">No CBT exams have been created.</div> : selectedExam && <div className="admin-list">{questions.map((q,i)=><div className="admin-card admin-question-card" key={q.id}><div className="admin-question-top"><span className="question-tag">Question {q.position}</span><div><button className="admin-text-btn" onClick={()=>startQuestion(q)}>Edit</button><button className="admin-text-btn danger-text" onClick={()=>void archiveQuestion(q.id)}>Remove</button></div></div><h3>{q.question_text}</h3><div className="admin-options"><div>A. {q.option_a}</div><div>B. {q.option_b}</div><div>C. {q.option_c}</div><div>D. {q.option_d}</div></div><small>Correct: <b>{q.correct_option}</b> · {q.marks} mark{q.marks === 1 ? '' : 's'}</small></div>)}{!questions.length && <div className="admin-card empty-state">No questions in this exam yet.</div>}</div>}
  </div></AdminLayout>;
}
