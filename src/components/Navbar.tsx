...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const navItems = [
    ...
    { id: 'my_school', label: 'My School & Vault', icon: BookOpen },
    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-4">
            ...
            >
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-extrabold uppercase px-1.5 py-0.5 rounded bg-orange-100 text-orange-800">
                    HUB
                  </span>
                </div>
                <p className="text-[10px] text-slate-500 font-semibold hidden sm:block">
                  Academic Vault & Campus Liaison
                </p>
              </div>
            </button>
          </div>
          ...
          {/* Right Action Controls: WhatsApp hotline + User Auth / Profile */}
          <div className="flex items-center gap-2.5">
            
            {/* Embedded WhatsApp Hotline with Clean Icon */}
            <a
              href={WHATSAPP_LINK}
              target="_blank"
              rel="noopener noreferrer"
              className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 hover:bg-emerald-100 text-xs font-bold transition-colors"
              title="Official WhatsApp Liaison Desk"
            >
              <div className="w-4 h-4 rounded-full bg-emerald-600 text-white flex items-center justify-center">
                <MessageSquare className="w-2.5 h-2.5" />
              </div>
              <span className="font-mono text-emerald-900">{WHATSAPP_DISPLAY}</span>
            </a>
            ...
            {isLoggedIn && user ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-3.5 py-1.5 text-xs font-bold bg-orange-600 hover:bg-orange-700 text-white rounded-xl shadow-xs transition-colors cursor-pointer"
                >
                  Register Free
                </button>
              </div>
        ...
        <div className="md:hidden border-t border-slate-200 bg-white p-4 space-y-3">
          ...
          <div className="pt-3 border-t border-slate-100 space-y-2">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-full py-2.5 px-3 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 font-bold text-xs flex items-center justify-center gap-2"
            >
              <MessageSquare className="w-4 h-4 text-emerald-600" />
              <span>WhatsApp Desk: {WHATSAPP_DISPLAY}</span>
            </a>
            ...
            {!isLoggedIn && (
              <div className="grid grid-cols-2 gap-2 pt-1">
                ...
                <button
                  ...
                  className="py-2.5 rounded-xl bg-orange-600 text-white text-xs font-bold text-center shadow-xs"
                >
                  Register Free
                </button>
              </div>
