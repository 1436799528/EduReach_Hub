...
export const MaterialCard: React.FC<MaterialCardProps> = ({
  ...
  return (
    <article id={`material-card-${material.id}`} className="er-card er-tilt rounded-2xl flex flex-col justify-between overflow-hidden group">
      <div className="p-5 pb-3">
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="px-2.5 py-1 rounded-md text-[11px] font-bold bg-slate-900 text-white flex items-center gap-1 shadow-sm">
      ...
      <div className="p-5 pt-3 mt-1 border-t border-slate-100 bg-slate-50/70">
        <div className="flex items-center justify-between mb-3 text-xs">
          <div className="flex items-center gap-2 min-w-0">
            <img src={material.uploader.avatar} alt={material.uploader.name} className="w-7 h-7 rounded-full object-cover border border-slate-200 shrink-0" referrerPolicy="no-referrer" />
            <div className="min-w-0"><p className="text-[11px] font-bold text-slate-800 leading-tight truncate">{material.uploader.name}</p><span className="text-[9px] text-slate-500 font-medium truncate block">{material.uploader.badge}</span></div>
          </div>
          <div className="flex items-center gap-2 text-[11px] text-slate-500 shrink-0"><span className="flex items-center text-amber-600 font-bold"><Star className="w-3 h-3 fill-amber-400 text-amber-400 mr-0.5" />{material.rating}</span><span className="text-slate-300">•</span><span>{material.unlockCount} unlocks</span></div>
        </div>

        {!isUnlocked && (
          <div className="mb-3 flex items-center gap-2 text-[11px] font-semibold text-slate-600 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
        ...
        <div className="flex items-center gap-2">
          <button id={`read-btn-${material.id}`} onClick={isUnlocked ? handleRead : handleUnlock} className={`er-cta flex-1 py-2.5 px-3 rounded-xl text-white text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${isUnlocked ? 'bg-slate-900 hover:bg-slate-800' : 'bg-orange-600 hover:bg-orange-700'}`}>
          ...
          {material.cbtQuestions && material.cbtQuestions.length > 0 && onOpenCBT && (
            <button id={`cbt-btn-${material.id}`} onClick={() => onOpenCBT(material)} className="er-cta p-2.5 rounded-xl bg-white hover:bg-slate-100 text-slate-800 transition-all border border-slate-300 text-xs font-bold flex items-center shadow-sm cursor-pointer" title="Practice CBT Exam Questions"><Play className="w-3.5 h-3.5 text-orange-600" /></button>
          )}

          <button id={`offline-btn-${material.id}`} onClick={() => onToggleOffline(material.id)} className={`er-cta p-2.5 rounded-xl transition-all border text-xs font-semibold cursor-pointer ${isSavedOffline ? 'bg-orange-50 border-orange-300 text-orange-800' : 'bg-white hover:bg-slate-100 border-slate-200 text-slate-600'}`} title={isSavedOffline ? 'Saved on your device (offline)' : 'Save for offline reading'}>
            {isSavedOffline ? <Check className="w-3.5 h-3.5 text-orange-600" /> : <DownloadCloud className="w-3.5 h-3.5 text-slate-500" />}
          </button>

          <button id={`share-btn-${material.id}`} onClick={handleShareWhatsApp} className="er-cta p-2.5 rounded-xl bg-white hover:bg-emerald-50 text-slate-600 hover:text-emerald-700 border border-slate-200 hover:border-emerald-300 transition-all cursor-pointer" title="Share to WhatsApp study group"><Share2 className="w-3.5 h-3.5" /></button>
        </div>
      </div>
    </article>
  );
