...
export default function HubSideRail() {
  ...
  return <aside className="hub-side-rail" aria-label="EduReach student tools">
    ...
    <section className="hub-rail-card">
      ...
      {!loadingServices && services.length > 0 && <div className="hub-rail-services">{services.slice(0, 5).map((service) => <a href={`/services/apply/${service.service_key}`} key={service.id}><span><strong>{service.title}</strong><small>{service.application_url ? 'Official portal available' : 'Request support'}</small></span><ArrowRight size={14}/></a>)}</div>}
    </section>
    ...
    <section className="hub-rail-card">
      ...
      {!loadingNews && news.length > 0 && <div className="hub-rail-news">{news.slice(0, 4).map((item) => <a href={`/news/${item.id}`} key={item.id}><span>{item.category.replaceAll('_', ' ')}</span><strong>{item.title}</strong><small>{formatDate(item.published_at)}</small></a>)}</div>}
