...
export async function updateMyProfile(userId: string, updates: Partial<Pick<SupabaseProfileRow, 'full_name' | 'school' | 'faculty' | 'department' | 'level' | 'session' | 'matric_number'>>) {
...
export function mapResourceToStudyMaterial(resource: ResourceRow, course?: SchoolCourseRow, institutionId: InstitutionId = 'ALL'): StudyMaterial { const level = course?.level && course.level >= 100 && course.level <= 500 ? `${course.level}L` as StudyMaterial['level'] : 'General'; return { id: resource.id, title: resource.title, courseCode: course?.code ?? 'RESOURCE', courseTitle: course?.title ?? 'Academic Resource', institutionId, department: '', faculty: '', level, semester: course?.semester === 1 ? '1st Semester' : course?.semester === 2 ? '2nd Semester' : 'All Year', materialType: toMaterialType(resource.resource_type), academicSession: '', unlockPrice: 0, storagePath: resource.storage_path ?? undefined, fileUrl: resource.file_url ?? undefined, externalUrl: resource.external_url ?? undefined, mimeType: resource.mime_type ?? undefined, uploader: { id: resource.uploaded_by ?? 'unknown', name: 'Verified Contributor', avatar: '', badge: 'Student Contributor', institution: institutionId, rating: 0 }, isVerified: resource.status === 'approved', verificationStatus: 'APPROVED', rating: 0, reviewCount: 0, unlockCount: 0, fileSizeKb: resource.file_size ? Math.round(resource.file_size / 1024) : 0, pageCount: 0, summary: resource.description ?? '', coreConcepts: [], fullTextContent: '', crossCampusEquivalents: [], createdAt: resource.created_at }; }

export function mapPastQuestionToStudyMaterial(question: PastQuestionRow, course?: SchoolCourseRow, institutionId: InstitutionId = 'ALL'): StudyMaterial { const level = course?.level && course.level >= 100 && course.level <= 500 ? `${course.level}L` as StudyMaterial['level'] : 'General'; const semester = question.semester ?? course?.semester ?? null; const session = question.session || `${question.year}`; const examType = question.exam_type ? ` • ${question.exam_type}` : ''; return { id: question.id, title: question.title, courseCode: course?.code ?? 'PAST QUESTION', courseTitle: course?.title ?? 'Past Questions', institutionId, department: '', faculty: '', level, semester: semester === 1 ? '1st Semester' : semester === 2 ? '2nd Semester' : 'All Year', materialType: 'past_question', academicSession: session, unlockPrice: 0, fileUrl: question.file_url ?? undefined, uploader: { id: 'edureach-academic-archive', name: 'EduReach Academic Archive', avatar: '', badge: 'Student Contributor', institution: institutionId, rating: 0 }, isVerified: question.status === 'approved', verificationStatus: 'APPROVED', rating: 0, reviewCount: 0, unlockCount: 0, fileSizeKb: 0, pageCount: 0, summary: `Approved ${question.year} ${question.title}${examType}.`, coreConcepts: [], fullTextContent: '', crossCampusEquivalents: [], createdAt: question.created_at }; }
...
export async function getCampusFeedPosts(): Promise<FeedPost[]> {
  ...
  const [{ data: authors }, { data: courses }, { data: likes }, { data: comments }] = await Promise.all([
    client.rpc('get_campus_feed_profiles', { p_ids: authorIds }),
  ...
  const authorMap = new Map((authors ?? []).map((author: { id: string; full_name: string; school: string; department: string; level: string }) => [author.id, author]));
